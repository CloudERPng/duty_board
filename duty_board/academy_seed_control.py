...
Seeded deliberately while INCOMPLETE. Two of nine modules are written, and the
...
Consequences of publishing early, and how each is handled:
  ...
  - The description says plainly that it is in preparation. If the flag is ever
    ...
    thin course.
  - Reconciles the module list on every run, so modules 5, 3, 4, 7, 8, 1 and 9
...
ORDER = ["getting_data", "revenue_pos"]
...
TRACK = {
    ...
    "seat_price": 85000,
    "description": (
        "IN PREPARATION — two of nine modules published for review. "
